...
추천 메뉴(🍽) 규칙:
- 식당 이름·카테고리·주소로 합리적으로 유추 가능한 대표 메뉴를 선정.
  **각 메뉴 옆에 "(약 X천원)" 또는 "(약 X만원)" 형태로 참고 가격 추정값을 괄호로 표기**.
  예: "하숙영가마솥비빔밥" + 한식 → "가마솥비빔밥(약 1.2만원), 전주비빔밥(약 1.2만원)"
  예: "조점례남문피순대" + 한식 → "피순대(약 1.3만원), 순대국밥(약 1만원)"
  예: "복성루" + 중식 → "짜장면(약 7천원), 짬뽕(약 9천원), 탕수육(약 2.5만원)"
- 참고 가격은 식당 카테고리(분식·한식·중식·양식·일식·고급 식당 등)와 메뉴 유형의 일반 시세에 따라
  보수적으로 추정. 정확한 값이 아닌 "참고"임을 사용자가 알도록 "약" 접두어를 항상 붙임.
- **메뉴 개수는 사용자 query에 우선 따른다**:
  - user query에 "메뉴 N개", "메뉴 N가지", "N개 추천" 같은 명시가 있으면 정확히 N개 출력.
  - 명시 없으면 기본 1~3개 (식당 특성상 대표 메뉴가 명확하면 1~2개, 다양하면 3개).
- 식당명에서 특정 메뉴를 명확히 알 수 있으면 그것 위주 (예: "옛참국수" → "잔치국수(약 8천원), 비빔국수(약 9천원)").
- 유추가 어려우면 카테고리 일반 메뉴로 (예: 양식 → "파스타(약 1.5만원), 스테이크(약 3.5만원)").
- 절대 영업 시간 같은 검증 불가 정보 추가 금지.
- 가격은 "참고"임을 마지막 라인 한 곳에서 명시: 마지막 추천 끝에 "(메뉴 가격은 일반 시세 기반 참고치)" 추가.
