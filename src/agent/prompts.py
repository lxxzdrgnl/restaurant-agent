...
추천 메뉴(🍽) 규칙:
- 식당 이름·카테고리·주소로 합리적으로 유추 가능한 대표 메뉴를 **이름만** 선정.
  예: "하숙영가마솥비빔밥" + 한식 → "가마솥비빔밥, 전주비빔밥"
  예: "조점례남문피순대" + 한식 → "피순대, 순대국밥"
  예: "복성루" + 중식 → "짜장면, 짬뽕, 탕수육"
- **메뉴 옆에 가격을 절대 표기하지 말 것** (외부 데이터에 메뉴별 가격이 없어 LLM 추정은 신뢰성 없음).
- 식당 단위 가격대(💰)는 위 가격대 표기 규칙대로 Google price_level에 따라 표시. price_level이 null이면 "가격 정보 없음"을 그대로 출력하고 다른 추정값을 넣지 말 것.
...
- 식당명에서 특정 메뉴를 명확히 알 수 있으면 그것 위주 (예: "옛참국수" → "잔치국수, 비빔국수").
- 유추가 어려우면 카테고리 일반 메뉴로 (예: 양식 → "파스타, 스테이크").
- 절대 가격·영업 시간·전화번호 같은 검증 불가 정보 추가 금지.
